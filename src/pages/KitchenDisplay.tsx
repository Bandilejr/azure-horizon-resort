                ...
                >
                  <div className="flex items-center justify-between mb-1">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs font-semibold text-gray-600">{getOrderTypeLabel(order.orderType)}</span>
                    </div>
      ...
      <Dialog open={showOrderModal} onOpenChange={setShowOrderModal}>
        <DialogContent className="max-w-lg bg-white border-none">
          ...
          <div className="space-y-4">
            <div className="flex items-center gap-2 text-sm text-gray-500 bg-slate-50 p-2 rounded">
...
function KanbanColumn({ 
  ...
  return (
    <div className="flex flex-col h-full bg-gray-200/50 rounded-lg border border-gray-200/80">
      ...
      <div className="flex-1 p-3 overflow-y-auto space-y-3">
        {orders.length === 0 ? (
          ...
          orders.map((order) => {
            ...
            return (
              <Card key={order.id} className="cursor-pointer hover:shadow-md transition-all active:scale-[0.98] border-none shadow-sm" onClick={() => onOrderClick(order)}>
                <CardContent className="p-4">
                  <div className="flex items-start justify-between mb-3">
                    <div className="flex items-center gap-2">
                      <div className="p-1.5 bg-slate-100 rounded text-slate-500">{getOrderTypeIcon(order.orderType)}</div>
                      <span className="text-[10px] font-bold text-slate-400 uppercase tracking-tight">{getOrderTypeLabel(order.orderType)}</span>
                    </div>
                    <span className={`text-[10px] font-bold uppercase px-2 py-1 bg-slate-50 rounded ${getUrgencyColor(elapsed)}`}>
                      {elapsedDisplay} min
                    </span>
                  </div>
                  <p className="font-bold text-sm text-[#1e3a5f]">{order.guestName}</p>
