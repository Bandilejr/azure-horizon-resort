...
export function LandingPage({ onRegisterClick }: LandingPageProps) {
  ...
  const handleGuestLogin = async () => {
    ...
    if (guestType === 'browsing') {
      ...
      if (!isValidEmail(guestEmail) || !guestPassword) {
        setErrorMessage("❌ Valid email and password required for residents.");
        return;
      }
      setIsLoggingIn(true);
      try {
        const result = await loginUser(guestEmail, guestPassword);
        if (result.user) {
          const userDoc = await getDoc(doc(db, 'users', guestEmail));
          if (userDoc.exists()) {
        ...
        console.error(err);
        setErrorMessage("❌ Invalid resident credentials. Please check your email and password.");
      } finally { setIsLoggingIn(false); }
    }
  };
  ...
  const handleStaffLogin = async () => {
    ...
    try {
      const result = await loginUser(staffEmail, staffPassword);
      ...
      const userDoc = await getDoc(doc(db, 'users', staffEmail));
      
      if (!userDoc.exists()) {
        setErrorMessage("❌ Staff profile not found. Please contact HR administrator.");
        await logoutUser();
        setIsLoggingIn(false);
        return;
      }
      
      const dbUser = userDoc.data() as CustomUser;
      const selectedRoleLower = selectedRole.toLowerCase();
      const userRoleLower = dbUser.role?.toLowerCase() || '';
      
      console.log("DB User:", dbUser);
      console.log("Selected role:", selectedRoleLower);
      console.log("User role from DB:", userRoleLower);
      ...
      if (userRoleLower !== selectedRoleLower) {
        ...
        setErrorMessage(`❌ Access Denied: ${staffEmail} is registered as "${expectedRoleName}", not "${selectedRoleName}".\n\nPlease select the correct department from the dropdown menu.`);
        await logoutUser();
        setIsLoggingIn(false);
        return;
      }
        ...
        <div className="relative max-w-7xl mx-auto px-4 pt-12 pb-20">
          ...
          <div className="grid lg:grid-cols-2 gap-8 max-w-5xl mx-auto">
            <Card className="bg-white/95 backdrop-blur-sm shadow-2xl border-0">
              <CardHeader>
                <CardTitle className="text-[#1e3a5f]">Secure Portal Access</CardTitle>
              ...
              <CardContent>
                ...
                <Tabs value={activeTab} onValueChange={setActiveTab}>
                  <TabsList className="grid w-full grid-cols-2 mb-6">
                    <TabsTrigger value="guest"><User className="h-4 w-4 mr-2" />Guest</TabsTrigger>
                    <TabsTrigger value="staff"><Users className="h-4 w-4 mr-2" />Staff</TabsTrigger>
                  </TabsList>

                  <TabsContent value="guest" className="space-y-6">
                    <div className="flex gap-2 p-1 bg-slate-100 rounded-lg">
                      <Button variant={guestType === 'browsing' ? 'default' : 'ghost'} className="flex-1 text-xs" onClick={() => setGuestType('browsing')}>
                        <Search className="h-3 w-3 mr-1" /> Browsing
                      </Button>
                      <Button variant={guestType === 'resident' ? 'default' : 'ghost'} className="flex-1 text-xs" onClick={() => setGuestType('resident')}>
                      ...
                      {guestType === 'browsing' ? (
                        ...
                        <div className="space-y-4 animate-in slide-in-from-top-2 duration-300">
                          <div className="space-y-2">
                            ...
                            <Input type="email" placeholder="j.doe@example.com" value={guestEmail} onChange={e => setGuestEmail(e.target.value)} />
                          </div>
                          <div className="space-y-2">
                            <Label>Password</Label>
                            <Input type="password" placeholder="••••••••" value={guestPassword} onChange={e => setGuestPassword(e.target.value)} />
                          </div>
                          <div className="space-y-2">
                            <Label className="text-xs text-gray-400">Room Number (Optional)</Label>
                            <Input placeholder="e.g. 101" value={roomNumber} onChange={e => setRoomNumber(e.target.value)} />
                          </div>
                        </div>
                      )}
                    </div>
                    <Button className="w-full bg-[#c9a227] hover:bg-[#b8941f] text-white" onClick={handleGuestLogin} disabled={isLoggingIn}>
                    ...
                    <div className="pt-4 border-t text-center">
                      <p className="text-sm text-slate-500 mb-3">Want to become a member?</p>
                      <Button variant="outline" className="w-full border-[#1e3a5f] text-[#1e3a5f] hover:bg-slate-50" onClick={onRegisterClick}>
                  ...
                  <TabsContent value="staff" className="space-y-4">
                    ...
                    <Input type="password" placeholder="Password" value={staffPassword} onChange={e => setStaffPassword(e.target.value)} />
                    <Button className="w-full bg-[#1e3a5f] hover:bg-[#152a45] text-white" onClick={handleStaffLogin} disabled={isLoggingIn}>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {features.map((f, i) => (
                <Card key={i} className="bg-white/10 backdrop-blur-sm border-white/20 text-white hover:bg-white/20 transition-all group">
                  <CardContent className="p-6">
                    <f.icon className="h-10 w-10 text-[#c9a227] mb-4 group-hover:scale-110 transition-transform" />
